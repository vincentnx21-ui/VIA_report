...
def save_data():
    ...
    try:
        ...
        for e in st.session_state.data.get("events", []):
            e_copy = e.copy()
            # DEFENSIVE: Check types before formatting
...
# --- MANAGEMENT ---
    ...
    with t1:
        ...
        for i, m in enumerate(st.session_state.data.get("members", [])):
            ...
            c1.write(f"**{m['name']}** {tag} - {m['project']}")
